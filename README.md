...
- [ ] 영수증 출력 후, 추가구매 진행 / 종료 여부 결정

### 계산

- [ ] 총 구매액 계산 (상품별 가격 X 수량)
- [ ] 최종 결제 금액 계산 _(👉🏻 프로모션 및 멤버십 할인 적용 금액)_

### 재고 관리

- [ ] 수량 관리
  - [ ] 상품 구매시 결제 수량만큼 재고 차감  
         👉🏻 `최신 재고 상태`를 유지하여 정확한 재고 정보 제공

### 프로모션 할인

- [ ] 오늘 날짜가 프로모션 기간에 해당하는 경우에만 적용
- [ ] N개 구매시 1개 증정 (`N+1`) 형태로 적용 _(❗️동일 상품에 여러 프로모션 적용 X❗️)_
  - [ ] `1+1` / `2+1` 프로모션이 각각 지정된 상품에 적용
- [ ] 프로모션 재고 내에서만 적용 가능
- [ ] 프로모션 기간
  - [ ] 프로모션 재고 우선 차감
  - [ ] 프로모션 재고 부족할 경우, 일반 재고 차감

### 프로모션 출력

- [ ] 프로모션 적용 상품에 대해, 프로모션 수량보다 적게 가져온 경우  
       👉🏻 필요 수량을 추가로 가져오면 혜택을 받을 수 있음을 안내
- [ ] 프로모션 재고가 부족한 경우
  - [ ] 일부 수량을 프로모션 혜택 없이 결재
...
- [ ] 멤버십 할인 `최대 한도`는 `8,000원`

### 영수증 출력

- [ ] 구매 내역 (상품명, 수량, 가격)
- [ ] 증정 상품 내역 _(👉🏻 프로모션에 따라 무료로 제공된 증정 상품 목록)_
- [ ] 금액 정보
  - [ ] 총 금액
  - [ ] 행사 할인 _(👉🏻 프로모션에 의해 할인된 금액)_
  - [ ] 멤버십 할인 _(👉🏻 멤버십에 의해 추가 할인된 금액)_
  - [ ] 내실 돈 _(👉🏻 최종 결제 금액)_
- [ ] 필드 칸 정렬
...
- [ ] 같은 제품을 여러번 입력한 경우 ex) `[콜라-1],[콜라-5]`
  - [ ] 서로 다른 입력을 합치기 ex) `[콜라-6]`
